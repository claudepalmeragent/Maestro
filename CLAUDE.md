...
| Honeycomb integration                    | `src/main/ipc/handlers/honeycomb.ts`, `src/main/services/honeycomb-*.ts`, `src/main/preload/honeycomb.ts`, `src/renderer/components/HoneycombSettingsSection.tsx`, `src/renderer/components/HoneycombWarningSash.tsx`                                            |
| GPU / system resource monitor            | `src/main/ipc/handlers/gpu-monitor.ts`, `src/main/utils/gpu-probe.ts`, `src/main/preload/gpuMonitor.ts`, `src/renderer/components/GpuResourcesPanel.tsx`                                                                                                         |
| Project Folders                          | `src/main/ipc/handlers/projectFolders.ts`, `src/renderer/components/sidebar/ProjectFolderHeader.tsx`, `src/renderer/components/modals/ProjectFolderModal.tsx`, `src/renderer/components/modals/ProjectFolderSettingsModal.tsx`                                   |
| Prompt Library                           | `src/main/prompt-library-manager.ts`, `src/main/ipc/handlers/prompt-library.ts`, `src/renderer/hooks/prompt-library/usePromptLibrary.ts`, `src/renderer/components/PromptLibrarySearchBar.tsx`, `src/renderer/components/PromptComposerModal.tsx`                |
| Conversation Knowledge Graph             | `src/main/ipc/handlers/knowledge-graph.ts`, `src/main/preload/knowledgeGraph.ts`, surfaced in `src/renderer/components/HistoryDetailModal.tsx`                                                                                                                   |
| Anthropic Audit / billing-mode breakdown | `src/main/services/anthropic-audit-service.ts`, `src/main/services/audit-scheduler.ts`, `src/main/ipc/handlers/audit.ts`, `src/main/preload/audit.ts`, `src/renderer/components/AuditResultModal.tsx`                                                            |
| Historical Reconstruction                | `src/main/services/historical-reconstruction-service.ts`, `src/main/ipc/handlers/reconstruction.ts`, `src/main/preload/reconstruction.ts`, `src/renderer/components/ReconstructionPreviewModal.tsx`                                                              |
| Like/Dislike Feedback                    | `src/main/ipc/handlers/feedback.ts`, `src/main/preload/feedback.ts`, `src/renderer/components/FeedbackModal.tsx`, `src/renderer/components/FeedbackChatView.tsx`                                                                                                 |
