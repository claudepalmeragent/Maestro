 ...
 *
 * Fork-only: preserved alongside upstream's later `loadFileTreeRemoteBatched`.
 * Returns a bare node array; callers wrap it into FileTreeLoadResult if needed.
 */
...
// Note: parseGitignoreContent is imported from '../../shared/globUtils' (top of file).
// The fork-local duplicate that previously lived here was removed during the
// upstream/main merge in favor of the shared implementation.
...
async function loadFileTreeRecursive(
	...
	try {
		...
		for (const entry of orderedEntries) {
			...
			// Safety exclusions (fork): these directories are ALWAYS skipped regardless
			// of user settings. .git/.git-repo exclusion prevents accidental repo
			// corruption; node_modules exclusion prevents thousands of sequential
			// SSH readDir calls that would exhaust the concurrency limiter.
			if (
			...
			// but never hide always-visible files (e.g. `.maestro`).
