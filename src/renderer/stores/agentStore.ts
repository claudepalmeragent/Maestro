...
export const useAgentStore = create<AgentStore>()((set, get) => ({
	...
	processQueuedItem: async (sessionId, item, deps) => {
		...
		try {
			...
			// Use cached agent config (renderer type) for yoloModeArgs if available
			const cachedAgent = get().getAgentConfig(session.toolType);
			const spawnArgs = isReadOnly
				? filterYoloArgs(agent.args || [], { yoloModeArgs: cachedAgent?.yoloModeArgs })
				: [...(agent.args || [])];
			...
			if (item.type === 'message' && (hasText || isImageOnlyMessage)) {
				...
				if (matchingCommand) {
					...
					// Clean up any $ARGUMENTS placeholders in the prompt
