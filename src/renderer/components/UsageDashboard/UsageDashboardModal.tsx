...
import type {
	...
	UsageDashboardViewMode as BaseViewMode,
} from '../../types';
// Extend the base view mode with fork-only 'dscomparison' tab (Honeycomb datasource comparison).
type ViewMode = BaseViewMode | 'dscomparison';
import {
...
export function UsageDashboardModal({
	...
	const [viewMode, setViewMode] = useState<ViewMode>(
		() =>
			(defaultViewMode as ViewMode) || (useUIStore.getState().usageDashboardViewMode as ViewMode)
	);
	...
	const fetchStats = useCallback(
		async (isRealTimeUpdate = false) => {
			...
			try {
				// Fetch stats, database size, cost data, and Cue totals in parallel.
				// Cue totals come from a separate stats system and only when the Cue
				// tab is enabled; a failure there must not break the rest of the
				// dashboard, so it resolves to null rather than rejecting the
				// Promise.all.
				const [stats, dbSize, dailyCosts, costsByModel, costsByAgent, cueAgg] = await Promise.all([
