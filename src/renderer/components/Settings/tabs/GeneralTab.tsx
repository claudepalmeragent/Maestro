...
export function GeneralTab({ theme, isOpen }: GeneralTabProps) {
	...
	useEffect(() => {
		...
		window.maestro.stats
			...
			.catch((err) => {
				logger.error('Failed to load stats database size:', undefined, err);
			});
		...
		window.maestro.stats
			...
			.catch((err) => {
				logger.error('Failed to load earliest stats timestamp:', undefined, err);
			});

		// Reset stats clear state when modal opens
		...
		<div className="space-y-5">
			...
			<div>
				...
				>
					...
					<div>
						<label className="block text-xs opacity-60 mb-2">Default time range</label>
						<select
							value={defaultStatsTimeRange}
							onChange={(e) =>
								setDefaultStatsTimeRange(e.target.value as typeof defaultStatsTimeRange)
							}
							...
							<button
								onClick={async () => {
									...
									try {
										...
										logger.error('Failed to clear old stats:', undefined, err);
										setStatsClearResult({
											success: false,
						...
						{statsClearResult && (
							...
							>
								{statsClearResult.success
									? `Cleared ${statsClearResult.deletedQueryEvents} query events, ${statsClearResult.deletedAutoRunSessions} Auto Run sessions, ${statsClearResult.deletedAutoRunTasks} tasks.`
									: `Error: ${statsClearResult.error}`}
							</p>
						)}
					</div>
				</div>
