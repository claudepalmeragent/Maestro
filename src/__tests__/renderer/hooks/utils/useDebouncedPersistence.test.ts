/**
 * Tests for useDebouncedPersistence hook and prepareSessionForPersistence.
 *
 * Covers:
 * - prepareSessionForPersistence direct tests (git detection fields, SSH runtime fields, etc.)
 * - Hook behavior (debounce, flush, initialLoadComplete gate)
 * - Wizard tab filtering, log truncation, tab runtime state reset
...
// ---------------------------------------------------------------------------
// Direct prepareSessionForPersistence tests (fork-specific)
// ---------------------------------------------------------------------------
...
describe('prepareSessionForPersistence', () => {
	...
	describe('other runtime fields are stripped', () => {
		it('should reset state to idle', () => {
			const session = makeSession({ state: 'busy' as any });
			const result = prepareSessionForPersistence(session);
...
// ---------------------------------------------------------------------------
// Hook-based test suite (upstream)
// ---------------------------------------------------------------------------
