 ...
 *
 * Verifies:
 * - Renders all thirteen metric cards correctly (including Total Cost, Open Tabs, Avg Throughput, Total Tokens)
...
const mockData: StatsAggregation = {
	...
	byAgent: {
		...
		codex: { count: 50, duration: 2200000, totalOutputTokens: 25000, avgTokensPerSecond: 20 },
	},
...
const onlyAutoData: StatsAggregation = {
	...
	byWorktreeStatus: {
		worktree: { count: 0, duration: 0 },
		parent: { count: 100, duration: 3600000 },
	},
	imageAnnotations: 0,
};
...
describe('SummaryCards', () => {
	describe('Rendering', () => {
		...
		it('renders all fifteen metric cards', () => {
			// Card count is 15: streak row added (Current Streak, Best Day, Active Days, Image Annotations)
			// plus fork extras (Avg Throughput, Total Tokens, Total Cost).
	...
	describe('Icons', () => {
		it('renders SVG icons for each metric', () => {
			...
			expect(svgElements.length - sparklines.length).toBe(15);
		});
	});
