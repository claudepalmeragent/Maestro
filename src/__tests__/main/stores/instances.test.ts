...
describe('stores/instances', () => {
	...
	describe('initializeStores', () => {
		it('should initialize all stores', () => {
			...
			// Should create 11 stores (HEAD's 10 + upstream's agentCapabilitiesStore for snapshot persistence)
			expect(mockStoreConstructorCalls).toHaveLength(11);
	...
	describe('getStoreInstances', () => {
		it('should return all store instances after initialization', () => {
			...
			expect(instances.projectFoldersStore).toBeDefined();
			expect(instances.modelRegistryStore).toBeDefined();
		});
	});
