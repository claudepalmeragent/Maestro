...
describe('Query events recorded for interactive sessions', () => {
	...
	describe('token metrics recording for interactive sessions', () => {
		it('should record token metrics with query event', async () => {
			const { StatsDB } = await import('../../../main/stats');
			const db = new StatsDB();
			db.initialize();
			...
			// Verify the INSERT was called with token parameters
			const runCalls = mockStatement.run.mock.calls;
			const lastCall = runCalls[runCalls.length - 1];
		...
		it('should correctly map token fields from database row', async () => {
			...
			// Verify snake_case -> camelCase mapping for token fields
			expect(event.inputTokens).toBe(200);
			expect(event.outputTokens).toBe(800);
			expect(event.tokensPerSecond).toBe(160);
		});

		it('should handle null token fields in database row', async () => {
			const now = Date.now();
			mockStatement.all.mockReturnValue([
				{
					id: 'no-token-event',
			...
			// Null values should map to undefined
