...
describe('loadFileTreeRemote', () => {
	...
	it('skips symlinks and special file types', async () => {
		const output = [
			'd\tsrc',
			'f\tsrc/index.ts',
			'l\tsrc/link.ts',
			'p\tmy-pipe', // named pipe - should be skipped
			's\tmy-socket', // socket - should be skipped
		].join('\n');
		...
		expect(result.success).toBe(true);
		// Should include dir, file, and symlink (3 entries), skip pipe and socket
	...
	it('includes ignore patterns in find command', async () => {
		...
		// Verify the SSH command includes the prune arguments
