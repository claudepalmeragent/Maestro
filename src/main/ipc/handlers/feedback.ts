 ...
 *
 * Fork-only `registerFeedbackHandlers()` (no-arg) records user feedback
 * (like/dislike) on AI responses to per-user Markdown files for future RLM
 * processing.
 *
 * Upstream's `registerGhFeedbackHandlers(deps)` provides a separate GitHub
 * issue submission flow (gh CLI auth check, attachment upload, issue create).
 */
 ...
 */
export function registerGhFeedbackHandlers(_deps: FeedbackHandlerDependencies): void {
...
// ============================================================================
// Fork-only: Like/Dislike feedback recording (no-arg handler)
// ============================================================================

const RLM_FEEDBACK_DIR = 'feedback';
...
async function ensureFeedbackDir(): Promise<string> {
	const feedbackDir = path.join(app.getPath('userData'), RLM_FEEDBACK_DIR);
...
/**
 * Register fork-only like/dislike feedback IPC handlers.
 */
