...
export async function routeModeratorResponse(
	...
	if (processManager && agentDetector && mentionsToSpawn.length > 0) {
		...
		for (const participantName of mentionsToSpawn) {
			...
			// `agent` may be null for SSH-remote participants whose definition is not
			// available locally; downstream spawn handles the remote case, so defensive
			// optional-chaining is correct here.
			...
			try {
				...
				const spawnResult = await spawnGroupChatAgent({
					...
					// agent may be null for SSH-remote participants whose definition
					// is not available on the local host; spawnGroupChatAgent's typed
					// signature expects non-null. The remote-resolution branch inside
					// the spawn helper is the one that actually executes for null
					// agents, so the assertion is safe for the local typecheck path.
					agent: agent!,
...
`;
	...
	const taskInstruction = roundState.forcedSynthesis
		? `## Your Task:
Round limit reached. You MUST synthesize all agent responses into a final answer for the user NOW. Do NOT use any @mentions, NO !autorun. Produce a complete summary.
