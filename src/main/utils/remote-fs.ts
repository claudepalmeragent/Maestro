...
/**
 * Reset the concurrency limiter for a single host. Called when SSH settings
 * change so the next call rebuilds with the new config. Retained as an
 * exported symbol because the upstream concurrency tests import it; our
 * fork-side HostLimiter is per-host with the same key shape.
 */
export function resetHostLimiter(config: SshRemoteConfig): void {
	hostLimiters.delete(sshHostKey(config));
}
...
export async function execRemoteCommand(
	...
	try {
		return await execRemoteCommandInner(config, remoteCommand, deps, input);
	} finally {
		limiter.release();
	}
}
...
export async function loadFileTreeRemote(
	...
	// Each pattern becomes: -name 'pattern' -prune
	// These prevent descending into ignored directories
