import { app, BrowserWindow, Menu, nativeTheme, powerMonitor, protocol } from 'electron';
...
app
	...
	.then(async () => {
		...
		// (history manager already initialized earlier in this Startup block; the
		// merge resolution left a duplicate of the init/watch sequence here which
		// caused a "Cannot redeclare block-scoped variable 'historyManager'" TS
		// error. The earlier block at ~L820 is the canonical one and is preserved.)
...
function setupIpcHandlers() {
	...
	registerAgentSessionsHandlers({
		...
		// store is ElectronStore<MaestroSettings>; agentSessions only needs the
		// minimal `{ get(key, default) }` surface, so accept the wider type.
		settingsStore: store as unknown as { get: (key: string, defaultValue?: unknown) => unknown },
	});
	...
	// Register GitHub-feedback handlers (gh auth + feedback submission)
	registerGhFeedbackHandlers({
		getProcessManager: () => processManager,
		debugPackageDeps: {
			getAgentDetector: () => agentDetector,
