...
- Claude Code's TUI supports injecting user messages mid-turn (between tool calls in its agentic loop), but this is not available in batch (`--print`) mode. Maestro runs Claude either through an interactive PTY wrapper (Max-plan TUI) or `--print` (API) per turn, and queues new messages until the current turn completes (sent via `--resume`). The mid-turn injection limit comes from the CLI's batch interface, not Maestro.
