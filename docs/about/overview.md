...
| **Provider Pass-Through** | Maestro delegates all AI work to your installed provider (Claude Code, Codex, OpenCode, Factory Droid, Copilot-CLI). Your MCP tools, custom skills, permissions, and authentication all carry over. Claude Code spawns either as an interactive PTY (Max-plan TUI) or in batch (`--print`) mode per turn; other providers run in their batch/exec equivalents. |
