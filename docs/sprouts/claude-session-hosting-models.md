...
- **Implementation:** Phase 3 `ClaudeBgAdapter` (`src/main/utils/claude-bg-adapter.ts`, ARDs C1-C7). The adapter mirrors `ClaudePtyRunner`'s public API surface so the dispatcher in `process.ts` / `agent-spawner.ts` can swap implementations transparently based on resolved transport mode.
...
- **Implementation:** Phase 2 `--ide` track (ARDs 2.I.1-2.I.4); composition with Model 2 deferred to Phase 3 (ARD 3.C.6).
...
- Model 2 + Model 3 = `background-supervisor` with optional `--ide` attach — supervisor-owned session, editor RPC active when Maestro is connected, gracefully degrades to plain when Maestro is closed. Phase 3.C.6.
...
| **Supervisor (Model 2)**  | `background-supervisor`                                                 | `background-supervisor-ide` (Phase 3.C.6 if shipped) |
...
- **Don't compose unless you've thought through the failure modes.** `--bg --ide` composition is two systems that can each independently fail; the cross-product matters. Hence ARD 3.C.6 gets its own design loop.
...
| Phase 1.5          | Defensive: prevent Model 2 hijacking Model 1 sessions                                                                                                | Posture A — A1                         | To author                        |
| Phase 2 Track B    | Coexist: surface Model 2 sessions in Maestro UI                                                                                                      | Posture B — B1, B2, B3, B4, B5         | To author                        |
| Phase 2 Track I    | Add Model 3 to Model 1                                                                                                                               | `--ide` — I1, I2, I3, I4               | To author                        |
| Phase 3            | Make Model 2 a first-class transport; compose with Model 3                                                                                           | Posture C — C1, C2, C3, C4, C5, C6, C7 | To author                        |
